  ...
  <div>
    ...
    >
      ...
      <UModal v-model="editProfileModal" v-if="editProfileModal">
        ...
        <div class="p-8 bg-white rounded-lg dark:bg-neutral-900 dark:text-white">
          <editProfile />
        </div>
      </UModal>
      <div
      ...
      <div class="text-center mt-2" v-if="!userStore().isLoading">
        ...
        <p class="text-gray-500">{{ user?.user_type }}</p>
        <!-- email  -->
        <p class="text-gray-500">{{ user?.email }}</p>
        <!-- date -->
        <p class="text-gray-500">{{ formatDate(user?.created_at) }}</p>
      </div>
      <!-- loading -->
      <div class="text-center mt-2" v-if="userStore().isLoading">
        <Icon name="eos-icons:bubble-loading" class="h-9 w-9" />
